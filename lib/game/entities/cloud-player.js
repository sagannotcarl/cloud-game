...
.defines(function() {
  EntityCloudPlayer = ig.Entity.extend({
    ...
    handleMovementTrace: function( res ) {
      if( res.collision.x && ig.game.playerController.endSequence == true) {
          ...
          if (ig.game.playerController.currentTreeHealth > 0) {
            var percentage = Math.ceil(ig.game.playerController.currentTreeHealth / ig.game.playerController.maxTreeHealth * 100);
            // ig.game.notificationManager.spawnNote('media/04b03black.font.png', 'Score: ' + percentage + '%', 9052, 200);
            // ig.game.notificationManager.spawnNote('media/04b03black.font.png', 'Score: ' + percentage + '%', ig.game.screen.x + (ig.system.width / 2), (ig.system.height / 2));
          }
          */
      }
