...
.defines(function() {
  EntityCloudPlayer = ig.Entity.extend({
    ...
    animSheet: new ig.AnimationSheet('media/cloud-player.png', 110, 65),
    ...
    update: function() {
      ...
      if (ig.input.state('rain')) {
        ...
        if (ig.game.lakeProximity && ig.game.getEntitiesByType( EntityEvaporation ).length === 0 && (this.pos.y + this.size.y) > (315 - 83)) {
          ...
          if (ig.game.playerController.waterLevel > 0) {
      ...
      var waterLevelAnim = (ig.game.playerController.waterLevel <= 7) ? (ig.game.playerController.waterLevel).floor() : 7;
