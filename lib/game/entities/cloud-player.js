...
.defines(function() {
  EntityCloudPlayer = ig.Entity.extend({
    ...
    update: function() {
      /* Controls */
      if ((ig.input.state('hold') && (ig.input.mouse.y < this.pos.y)) || ig.input.state('up')) {
        this.vel.y = -50;
      }
      else if ((ig.input.state('hold') && this.pos.y < (280 - 95)  && (ig.input.mouse.y >= this.pos.y)) || (ig.input.state('hold') && this.pos.y < (280 - 95))) { // Don't let the cloud touch the ground
        this.vel.y = 50;
      } 
      else {
        this.vel.y = 0;
