...
MyGame = ig.Game.extend({
	...
	init: function() {
		...
		/* Store a variable with the location of the lakes */
		var allLakes = this.getEntitiesByType( EntityLake );
		var allLakePositions = {};
    
    for (var index in allLakes) {
      if (allLakes[index].pos) {
        allLakePositions[index] = allLakes[index].pos.x;
      }
    }
    
    console.log(allLakePositions);
